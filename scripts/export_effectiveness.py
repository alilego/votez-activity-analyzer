#!/usr/bin/env python3
"""
Export word- and letter-weighted effectiveness metrics from the current SQLite state.
...
DEFAULT_OUTPUT_DIR = Path("outputs/effectiveness")
...
def _finalize_stats(stats: dict[str, Any]) -> dict[str, Any]:
    ...
    out = dict(stats)
    out["word_effectiveness_ratio"] = word_ratio
    out["word_effectiveness_pct"] = round(word_ratio * 100, 2) if word_ratio is not None else None
    out["letter_effectiveness_ratio"] = letter_ratio
    out["letter_effectiveness_pct"] = round(letter_ratio * 100, 2) if letter_ratio is not None else None
    # Backwards-compatible alias for the original word-based metric.
    out["effectiveness_ratio"] = out["word_effectiveness_ratio"]
    out["effectiveness_pct"] = out["word_effectiveness_pct"]
    return out
...
def _sort_effectiveness(entries: list[dict[str, Any]], id_key: str) -> list[dict[str, Any]]:
    return sorted(
        entries,
        key=lambda x: (
            -(x.get("word_effectiveness_pct") if x.get("word_effectiveness_pct") is not None else -1),
            -int(x.get("total_word_count", 0)),
            str(x.get(id_key) or ""),
        ),
    )
...
def export_effectiveness(
    *,
    ...
    for member_id, data in sorted(members.items()):
        ...
        member_path = members_dir / f"effectiveness_{member_id}_{member_slug}.json"
        ...
        member_index.append(
            {
                ...
                "word_effectiveness_ratio": finalized["word_effectiveness_ratio"],
                "word_effectiveness_pct": finalized["word_effectiveness_pct"],
                "letter_effectiveness_ratio": finalized["letter_effectiveness_ratio"],
                "letter_effectiveness_pct": finalized["letter_effectiveness_pct"],
                "effectiveness_ratio": finalized["effectiveness_ratio"],
                "effectiveness_pct": finalized["effectiveness_pct"],
            }
        )

    member_index = _sort_effectiveness(member_index, "member_id")
    (members_dir / "effectiveness_index.json").write_text(
    ...
    for party_id, data in sorted(parties.items()):
        ...
        member_summaries = _sort_effectiveness(member_summaries, "member_id")
        finalized = _finalize_stats(data)
        ...
        party_path = parties_dir / f"effectiveness_{party_id}.json"
    ...
    party_index = _sort_effectiveness(party_index, "party_id")
    (parties_dir / "effectiveness_index.json").write_text(
    ...
    (output_dir / "effectiveness_total.json").write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export word- and letter-weighted effectiveness percentages from current DB state."
    )
    parser.add_argument(
        "--db-path",
    ...
    print("Outputs:")
    print(f"  Total:   {output_dir / 'effectiveness_total.json'}")
    print(f"  Members: {output_dir / 'members' / 'effectiveness_index.json'}")
    print(f"           {output_dir / 'members' / 'effectiveness_{member_id}_{name_slug}.json'}")
    print(f"  Parties: {output_dir / 'parties' / 'effectiveness_index.json'}")
    print(f"           {output_dir / 'parties' / 'effectiveness_{party_id}.json'}")
    print("Format:")
    print("  Each entry includes member_id/name/party_id/party_name when applicable,")
    print("  processed and constructive intervention counts, total/constructive word")
    print("  counts, total/constructive letter counts, and word/letter effectiveness")
